...
> = () => {
  ...
  const { data: tokenBalanceGCOW } = useTokenBalance(
    tokenContractGCOW,
    address
  );
  ...
  const {
    ...
    isLoading,
    error,
  } = useTransferToken(tokenContractGCOW);
  ...
  async function transferToken(toAddress: string, amount: number) {
    ...
    try {
      /*
      const tx = await tokenContractGCOW?.erc20.transfer.prepare(toAddress, amount);
      ...
      */
      ...
      const transaction = await tokenContractGCOW?.erc20.transfer(
        toAddress,
        amount
      );
  ...
  return (
    <>
      ...
      <div className="flex-cols mt-5 flex items-center justify-center gap-3 rounded-lg bg-sky-600 pb-5 pt-5 text-white">
        <div className="text-2xl font-bold">GCOW</div>
      </div>
      ...
      <form>
        <div className=" flex flex-row items-center justify-center text-lime-600">
          ...
          <div className={styles.collectionContainer}>
            <div className="mb-2 text-lg">Send my GCOW to another address:</div>
            ...
            */}
            ...
            <input
              ...
              onChange={(e) => {
                if (e.target.value === null) setAmount(undefined);
                  ...
                  Number(tokenBalanceGCOW?.displayValue)
                ) {
                  setAmount(Number(tokenBalanceGCOW?.displayValue));
                } else {
            ...
            {address && (
              <div className="mb-3 text-lg">
                {(
                  Number(tokenBalanceGCOW?.displayValue) - (amount || 0)
                ).toFixed(2)}{' '}
                {tokenBalanceGCOW?.symbol} left
              </div>
            )}
                ...
                {isSending ? (
                  <div className="flex flex-row items-center justify-center gap-2">
                    ...
                    <div className="flex flex-col items-center justify-center text-2xl font-bold text-orange-600">
                      <span>
                        Sending {amount} {tokenBalanceGCOW?.symbol} to
                      </span>
                  ...
                  <>
                    <Web3Button
                      theme="light"
                      contractAddress={tokenContractAddressGCOW}
                      action={(contract) => {
      ...
      <div className="mx-auto mt-8 flex w-full shrink-0 flex-col md:px-4 xl:px-6 3xl:max-w-[1700px] 3xl:px-12">
        <TransactionTable {...{ contractAddress: tokenContractAddressGCOW }} />
      </div>
