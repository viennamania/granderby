...
> = () => {
  ...
  const { contract: tokenContractGSUGAR } = useContract(
    tokenContractAddressSUGARDrop,
    'token'
  );
  ...
  const {
    ...
    isLoading,
    error,
  } = useTransferToken(tokenContractGSUGAR);
  ...
  async function transferToken(toAddress: string, amount: number) {
    ...
    try {
      ...
      */
      ...
      const transaction = await tokenContractGSUGAR?.erc20.transfer(
        toAddress,
        amount
      );
  ...
  return (
    <>
      ...
      <div className="flex-cols mt-5 flex items-center justify-center gap-3 rounded-lg bg-sky-600 pb-5 pt-5 text-white">
        <div className="text-2xl font-bold">{tokenBalanceSUGAR?.symbol}</div>
      </div>
        ...
        <div className=" flex flex-row items-center justify-center text-lime-600">
          ...
          <div className={styles.collectionContainer}>
            <div className="mb-2 text-lg">
              Send my {tokenBalanceSUGAR?.symbol} to another address:
            </div>
            ...
            */}
            ...
            <input
              ...
              onChange={(e) => {
                if (e.target.value === null) setAmount(undefined);
                  ...
                  Number(tokenBalanceSUGAR?.displayValue)
                ) {
                  setAmount(Number(tokenBalanceSUGAR?.displayValue));
                } else {
            ...
            {address && (
              <div className="mb-3 text-lg">
                {(
                  Number(tokenBalanceSUGAR?.displayValue) - (amount || 0)
                ).toFixed(2)}{' '}
                {tokenBalanceSUGAR?.symbol} left
              </div>
            )}
                ...
                {isSending ? (
                  <div className="flex flex-row items-center justify-center gap-2">
                    ...
                    <div className="flex flex-col items-center justify-center text-2xl font-bold text-orange-600">
                      <span>
                        Sending {amount} {tokenBalanceSUGAR?.symbol} to
                      </span>
                  ...
                  <>
                    ...
                    >
                      Transfer ({amount} {tokenBalanceSUGAR?.symbol})
                    </Web3Button>
                  </>
                )}
