...
> = () => {
  ...
  const { contract: tokenContractGCARROT } = useContract(
    tokenContractAddressCARROTDrop,
    'token'
  );

  const { data: tokenBalanceCARROT } = useTokenBalance(
    tokenContractGCARROT,
    address
  );
  ...
  const {
    ...
    isLoading,
    error,
  } = useTransferToken(tokenContractGCARROT);
  ...
  async function transferToken(toAddress: string, amount: number) {
    ...
    try {
      ...
      */
      ...
      const transaction = await tokenContractGCARROT?.erc20.transfer(
        toAddress,
        amount
      );
  ...
  return (
    <>
      ...
      <div className="flex-cols mt-5 flex items-center justify-center gap-3 rounded-lg bg-sky-600 pb-5 pt-5 text-white">
        <div className="text-2xl font-bold">{tokenBalanceCARROT?.symbol}</div>
      </div>
        ...
        <div className=" flex flex-row items-center justify-center text-lime-600">
          ...
          <div className={styles.collectionContainer}>
            <div className="mb-2 text-lg">
              Send my {tokenBalanceCARROT?.symbol} to another address:
            </div>
            ...
            */}
            ...
            <input
              ...
              onChange={(e) => {
                if (e.target.value === null) setAmount(undefined);
                  ...
                  Number(tokenBalanceCARROT?.displayValue)
                ) {
                  setAmount(Number(tokenBalanceCARROT?.displayValue));
                } else {
            ...
            {address && (
              <div className="mb-3 text-lg">
                {(
                  Number(tokenBalanceCARROT?.displayValue) - (amount || 0)
                ).toFixed(2)}{' '}
                {tokenBalanceCARROT?.symbol} left
              </div>
            )}
                ...
                {isSending ? (
                  <div className="flex flex-row items-center justify-center gap-2">
                    ...
                    <div className="flex flex-col items-center justify-center text-2xl font-bold text-orange-600">
                      <span>
                        Sending {amount} {tokenBalanceCARROT?.symbol} to
                      </span>
                  ...
                  <>
                    ...
                    >
                      Transfer ({amount} {tokenBalanceCARROT?.symbol})
                    </Web3Button>
                  </>
                )}
