...
export default async function handler(
  ...
  //console.log('getClaimHistoryByTokenId tokenId', tokenId);
  ...
  const result = await fetch(
    ...
    `http://3.38.2.94:3001/api/horse/claimHistory?uid=${uid}`
  );

  const json = await result.json();

  //console.log('getClaimHistoryByTokenId json', json);

  res.status(200).json({
    claimHistory: json?.recordset,
  });
}
